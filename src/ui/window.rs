...
pub fn build_ui(app: &adw::Application) {
    ...
    let window = adw::ApplicationWindow::builder()
        ...
        .width_request(960)
        .height_request(400)
        .icon_name("com.mdnotebook.editor")
        .build();
...
pub fn build_editor(
    ...
    apply_sourceview_theme(&md_view, is_dark_active());
    ...
    let ctx = EditorCtx {
        ...
        rich_buffer: rich_buffer.clone(),
        rich_panel: rich_scroll,
        markdown_buffer: md_buffer,
        markdown_view: md_view,
        markdown_panel: md_scroll,
        split,
        status,
        doc_label,
        dirty_label,
    ...
    wire_editor_signals(&ctx, &rich_view, &tag_entry);
    wire_keyboard_shortcuts(&ctx, &rich_view, window);
    wire_close_request(&ctx);
    setup_auto_save(&ctx);

    // Wire source toggle button in content header
    {
        let ctx = ctx.clone();
        let btn = source_toggle.clone();
        source_toggle.connect_clicked(move |_| toggle_source_mode(&ctx, &btn));
    }
...
pub fn build_content_pane() -> ContentPaneWidgets {
    ...
    // End: source toggle + theme toggle + primary menu
    let source_toggle = gtk::Button::from_icon_name("text-x-generic-symbolic");
    source_toggle.set_tooltip_text(Some("Toggle source mode"));
    source_toggle.add_css_class("flat");
    source_toggle.set_visible(false); // shown only in zen mode (toolbar has MD/RT pill)
    set_accessible_label(&source_toggle, "Toggle source mode");
    content_header.pack_end(&source_toggle);
    ...
    // Editor panes
    let (rich_buffer, rich_view, rich_scroll) = build_rich_editor();
    let (md_buffer, md_view, md_scroll) = build_markdown_pane();

    let split = gtk::Paned::new(gtk::Orientation::Horizontal);
    split.set_wide_handle(true);
    split.set_position(700);
    split.set_start_child(Some(&rich_scroll));
    split.set_end_child(Some(&md_scroll));
    ...
    // Tags row
    let (tags_row, tags_box, tag_entry) = build_tags_row();
    content_box.append(&tags_row);

    content_toolbar_view.set_content(Some(&content_box));

    // --- Bottom bar: ActionBar with word count + metadata ---
...
pub fn build_rich_editor() -> (gtk::TextBuffer, gtk::TextView, gtk::ScrolledWindow) {
    let buffer = gtk::TextBuffer::new(None::<&gtk::TextTagTable>);
    wysiwyg::install_tags(&buffer);
    {
        wysiwyg::load_markdown(&buffer, DEFAULT_DOC);
    }
    ...
    view.add_css_class("wysiwyg-pane");
...
pub fn build_markdown_pane() -> (sourceview::Buffer, sourceview::View, gtk::ScrolledWindow) {
    ...
    view.set_vexpand(true);
    view.set_hexpand(true);
    view.add_css_class("markdown-pane");
...
pub fn build_tags_row() -> (gtk::Box, gtk::FlowBox, gtk::Entry) {
    let row = gtk::Box::new(gtk::Orientation::Horizontal, 6);
    row.add_css_class("tags-row");
