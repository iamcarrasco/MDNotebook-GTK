...
pub fn show_shortcuts_window(parent: &adw::ApplicationWindow) {
    ...
    for (group_title, entries) in shortcuts {
        let group_label = gtk::Label::new(Some(group_title));
        ...
        for (accel, desc) in entries.iter() {
...
fn build_help_main_page(
    ...
    let intro = gtk::Label::new(Some(
        "MD Notebook is a private, offline, encrypted Markdown notebook. \
...
fn build_vault_page() -> adw::NavigationPage {
    ...
    c.append(&help_paragraph(
        "MD Notebook stores all your notes in a single encrypted vault file. \
...
fn build_formatting_page() -> adw::NavigationPage {
    ...
    c.append(&help_paragraph(
        "Use the toolbar or keyboard shortcuts to format text. \
         The editor shows a rich-text preview; toggle the MD/RT pill \
         in the toolbar to switch to raw Markdown source view."
    ));
...
fn build_code_blocks_page() -> adw::NavigationPage {
    ...
    c.append(&help_paragraph(
        "The opening and closing fence markers (```) are non-editable. \
         Type your code between them. The block is rendered in a monospace font."
    ));
...
fn build_tables_page() -> adw::NavigationPage {
    ...
    c.append(&help_tip(
        "Tip: For complex tables, switch to Markdown source view (MD/RT toggle) \
         where you can edit the raw pipe-delimited syntax directly."
    ));
