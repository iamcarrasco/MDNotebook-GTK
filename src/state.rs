...
pub const PAGE_WELCOME: &str = r#"# Welcome to MD Notebook

MD Notebook is a private, offline markdown notebook. Your notes are encrypted with AES-256-GCM and never leave your machine — no cloud, no accounts, no tracking.

## Getting started

1. **Write** using the rich editor — formatting is applied as you type
...
5. **Export** any note as Markdown or HTML from the menu
...
See the other built-in notes for formatting examples, keyboard shortcuts, and Mermaid diagrams.
"#;
...
- **Backlinks** — see which notes link to the current one using `[[wiki-link]]` syntax
- **Tabs** — open multiple notes in tabs, drag to reorder, close with Ctrl+W
- **Export** — save any note as Markdown (.md) or HTML (.html) from the menu
...
pub struct DocState {
    ...
    pub cached_key: Option<crypto::CachedKey>,
}
...
impl DocState {
    ...
    /// Returns `None` if the active note wasn't changed, or if deletion failed.
...
mod tests {
    ...
    fn test_move_note_to_trash_active_tab() {
        ...
        let initial_count = state.notes.len(); // 4 default notes
    ...
    #[test]
    fn test_move_note_to_trash_inactive_tab() {
        let mut state = DocState::default();
        let initial_count = state.notes.len(); // 4 default notes
        state.notes.push(NoteItem::new(
            "note-extra".to_string(),
        ...
        state.active_note_id = "note-1".to_string();
        state.open_tabs = vec!["note-1".to_string(), "note-extra".to_string()];

        let switch = state.move_note_to_trash("note-extra");

        assert_eq!(switch, None); // Should not need to switch tabs
        assert_eq!(state.notes.len(), initial_count);
        assert_eq!(state.trash.len(), 1);
        assert_eq!(state.open_tabs, vec!["note-1".to_string()]);
    }
    ...
    fn test_move_note_to_trash_last_note() {
        let mut state = DocState::default();
        // Remove all but one note to test single-note protection
        ...
        assert_eq!(state.notes.len(), 1); // Should deny deleting the last note
